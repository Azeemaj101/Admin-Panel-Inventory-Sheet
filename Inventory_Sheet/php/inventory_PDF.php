...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['DOS'])) {
    $Start = $_POST['DOS'];
    $End = $_POST['DOE'];
    $Que = "SELECT *FROM `inventory_t` where `DATE` BETWEEN '$Start' AND '$End'";
    ...
    if (mysqli_num_rows($RUN)) {
        if (mysqli_num_rows($RUN) > 0) {
            ...
            $html .= '<tr><th scope="col">#SR</th><th scope="col">Username</th><th scope="col">Iteam Name</th><th scope="col">Date</th><th scope="col">Description</th><th scope="col">Quantity</th><th scope="col">Estimated</th></tr>';
            $i = 0;
            while ($row = mysqli_fetch_assoc($RUN)) {
                $i += 1;
                $html .= '<tr><td>' . $i . '</td><td>' . $row['USERNAME'] . '</td><td>' . $row['ITEAM_NAME'] . '</td><td>' . $row['DATE'] . '</td><td>' . $row['DESCRIPTION'] . '</td><td>' . $row['QUANTITY'] . '</td><td>' . $row['ESTIMATED_COST'] . '</td></tr>';
            }
...
<body style="background-color: gray;" id="body-pd">
    ...
    <form action="/inventory_sheet/php/inventory_PDF.php" method="POST">
        <div class="d-flex justify-content-end m-5">
            <a href="/inventory_Sheet/php/inventory_Table.php"><button type="button" class="btn btn-outline-light text-dark mx-2 btn-lg"><strong>
                        <ion-icon name="arrow-back"></ion-icon>
                    </strong></button></a>
            <button type="submit" class="btn btn-warning btn-lg" name='Download'>
